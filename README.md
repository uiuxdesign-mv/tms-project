...
### Fase 10 (UX & Polish) — selesai

Tindak lanjut bagian "Fase 10 — UX & Polish" di roadmap audit. Setelah Fase 0–9 menyelesaikan seluruh modul fungsional, fase ini fokus ke pengalaman pakai sehari-hari: navigasi yang tidak berkedip antar halaman, notifikasi yang tidak memblokir browser, ringkasan visual di Dashboard, laporan yang bisa diekspor ke format kantor (Excel/PDF), tabel yang tetap enak dipakai walau datanya sudah banyak, dan pilihan tema/bahasa. Cakupan (termasuk i18n & dark mode) dikonfirmasi langsung ke stakeholder sebelum implementasi — 6 sub-item berikut semuanya masuk cakupan, tidak ada yang di-skip.

- ✅ **Navigasi shell persisten** — seluruh halaman setelah login (`Dashboard`, `Tasking`, `Report`, `Master Data`, `Administrasi`, `Profil`) dipindah ke Next.js route group `src/app/(app)/` dengan 1 `layout.tsx` server yang menghitung sidebar+topbar sekali dari sesi & Menu Access (route group tidak mengubah URL, hanya struktur folder). Sidebar & topbar (`src/components/app-shell.tsx`) sekarang **benar-benar tidak remount** saat pindah halaman dalam grup ini (sebelumnya tiap halaman membangun headernya sendiri-sendiri dengan link "← Kembali ke Dashboard" berulang di 9 halaman) — navigasi terasa instan tanpa kedipan, dan state seperti tema/bahasa yang aktif tidak pernah "reset" saat pindah menu. Sidebar mendukung grup collapse/expand, highlight menu aktif, dan mode mobile (hamburger + overlay).
- ✅ **Sistem toast & confirm dialog** (`src/components/toast-provider.tsx`, `src/components/confirm-provider.tsx`) — mengganti **seluruh** pemakaian `alert()`/`confirm()` bawaan browser (memblokir thread, tidak bisa distyle, dan tidak konsisten antar browser) di 6 komponen (Comments, Users, Tasks, Time Tracking, Kanban, Master Data) dengan toast non-blocking (auto-hilang 4-6 detik, pojok kanan bawah) dan modal konfirmasi async (varian merah untuk aksi hapus).
- ✅ **Dashboard lengkap** — 6 chart ringkasan (Berdasarkan Status, Berdasarkan Prioritas, Berdasarkan Tipe Tugas, Top Assignee, Status Penyelesaian, Tren Jatuh Tempo Mingguan 10 minggu) + 4 feed card (**Aktivitas Terbaru** — mewire Audit Log yang sudah ada sejak Fase 6 jadi feed real-time, admin-only; Komentar Terbaru; Tugas Jatuh Tempo Segera 14 hari; Tugas Terbaru). Fungsi ringkasan (`src/lib/reports/summarize.ts`) diperluas dengan 3 breakdown baru (tipe tugas, assignee, tren mingguan) tanpa duplikasi logika — fungsi murni yang sama dipakai ulang oleh Dashboard (server-rendered) dan Report (client-filtered).
- ✅ **Report lengkap** — filter **Period** baru (Minggu Ini/Bulan Ini/Semua/Custom, di atas filter tanggal manual yang sudah ada), **Export Excel** (`.xlsx` asli lewat library `xlsx`, bukan cuma CSV seperti sebelumnya) dan **Export PDF** (lewat `jspdf`+`jspdf-autotable`, landscape, termasuk ringkasan angka di header), tombol **Print** (`window.print()` dengan stylesheet cetak yang menyembunyikan sidebar/topbar/filter, hanya menyisakan isi laporan), dan chart baru **Tren Jatuh Tempo Mingguan** (sama dengan yang di Dashboard). Kedua library export di-`import()` dinamis (bukan di top-level) supaya tidak membengkakkan bundle JS awal untuk pengguna yang tidak pernah klik Export.
- ✅ **Search/sort/pagination** di semua tabel data (Tasks, Master Data generik, Master Users) — hook generik `useTableControls()` (`src/lib/hooks/use-table-controls.ts`) menangani pencarian substring (hanya di kolom teks yang relevan, kolom ID/lookup sengaja dikecualikan supaya tidak mencocokkan ID mentah yang tidak berarti), sort klik-header (▲/▼, `localeCompare` locale Indonesia), dan paginasi 20 baris/halaman — dipakai identik di 3 tabel yang bentuknya berbeda-beda tanpa duplikasi logika.
- ✅ **i18n (ID/EN) & Dark Mode** — toggle bahasa dan tema di topbar (ikon 🌙/☀️ dan tombol ID/EN), tersimpan ke `localStorage` dan bertahan lintas reload/sesi, dengan inline script anti-kedip di `<head>` (menerapkan class/atribut **sebelum** React sempat hydrate, supaya tidak ada sekilas tampilan tema/bahasa salah sebelum berganti ke yang benar).
  - **Dark mode** diterapkan lewat **1 blok override CSS variable** (`.dark { --color-white: ...; --color-gray-50: ...; ... }` di `src/app/globals.css`), memanfaatkan fakta bahwa Tailwind v4 meng-compile setiap utility jadi referensi `var(--color-*)` (bukan nilai statis) — jadi seluruh UI yang sudah dibangun sejak Fase 0 otomatis ikut berubah ke versi gelap yang koheren, **tanpa perlu mengedit className `dark:` di tiap komponen satu per satu**. Token warna dasar (`--tms-*`) sengaja dipisah immutable dari token publik `--color-*` untuk menghindari bug resolusi CSS variable melingkar saat rentang warna dibalik (terang↔gelap).
  - **i18n** cakupannya **sengaja dibatasi** ke permukaan yang paling sering dilihat: chrome navigasi (sidebar+topbar), halaman Dashboard, dan halaman Login — menerjemahkan seluruh string di semua form/tabel Master Data, Tasks, dan Report adalah pekerjaan besar tersendiri di luar cakupan fase ini. Infrastrukturnya (`src/lib/i18n/translations.ts`, `src/components/language-provider.tsx`, hook `useLanguage()`) generik dan siap dipakai untuk memperluas cakupan itu kapan saja — tinggal menambah entry kamus baru dan memanggil `t('key')` di komponen yang bersangkutan. Nama entity Master Data (Clients, Projects, dst., dibangun dinamis dari konfigurasi Menu Access) tetap Indonesia-only, mengikuti batas cakupan yang sama.
- ✅ **Sudah diuji**: build produksi (`next build`) bersih tanpa error/warning baru. Diuji lewat headless browser (Playwright, dipasang sementara & dilepas lagi setelah selesai, tidak masuk `package.json`) dengan sesi admin asli: shell (`<aside>` sidebar) tampil di semua 11 halaman terautentikasi dan **tidak** tampil di `/login`; akses tanpa sesi tetap redirect ke `/login`; toggle dark mode mengubah class `.dark` di `<html>` dan warna teks/background berubah nyata (bukan cuma class kosong), bertahan setelah reload halaman; toggle bahasa ke EN mengubah label nav & heading Dashboard (mis. "Aktivitas Terbaru" → "Recent Activity", "Keluar" → "Log Out") dan bertahan setelah reload; **kedua preferensi tetap aktif saat pindah halaman** (membuktikan shell benar-benar persisten, bukan cuma re-render mirip); tabel Tasks/Master Data: search box ada & berfungsi, klik header sort tidak crash, teks paginasi tampil benar; halaman Report: dropdown Period, tombol Excel/PDF/Print semuanya ada dan export Excel/PDF menghasilkan file unduhan asli (diverifikasi lewat test terpisah di Fase 10 development) dengan isi data yang benar; nol error console/halaman di seluruh pengujian.
...
## Yang perlu dilakukan setelah deploy Fase 10

Tidak ada environment variable atau kolom sheet baru untuk Fase 10 — murni penambahan UI/UX di atas data yang sudah ada. Cukup deploy seperti biasa. Satu-satunya hal yang perlu diketahui pengguna: preferensi **tema (terang/gelap)** dan **bahasa (ID/EN)** disimpan per-browser lewat `localStorage`, jadi tidak ikut pindah kalau user login dari perangkat/browser lain — ini sesuai desain (bukan preferensi akun di server), bukan bug.
...
- `src/app/(app)/layout.tsx` (Fase 10) — layout server persisten untuk semua halaman setelah login, menghitung navGroups (termasuk translation key tiap item) sekali dari sesi & Menu Access.
- `src/components/app-shell.tsx` (Fase 10) — sidebar+topbar client component: highlight menu aktif, collapse/mobile toggle, toggle tema & bahasa, resolusi label nav lewat `t()`.
- `src/components/toast-provider.tsx`, `src/components/confirm-provider.tsx` (Fase 10) — pengganti `alert()`/`confirm()` bawaan browser, dipasang sekali di root layout lewat React Context.
- `src/components/theme-provider.tsx`, `src/components/language-provider.tsx`, `src/lib/i18n/translations.ts` (Fase 10) — infrastruktur dark mode & i18n: script anti-kedip di `<head>`, context/hook `useTheme()`/`useLanguage()`, kamus terjemahan ID/EN.
- `src/lib/hooks/use-table-controls.ts`, `src/components/table-controls.tsx` (Fase 10) — hook generik search/sort/pagination + komponen UI-nya (`SortableHeader`, `TableSearchBox`, `PaginationBar`), dipakai di semua tabel data.
- `src/components/charts/bar-list.tsx`, `src/components/charts/weekly-trend-chart.tsx` (Fase 10) — komponen chart presentasional (tanpa hook, aman di server) dipakai bersama oleh Dashboard & Report.
- `src/components/dashboard-view.tsx` (Fase 10) — client component render Dashboard (dipisah dari `page.tsx` server yang fetch data) supaya bisa memakai `useLanguage()` untuk terjemahan chrome Dashboard.
